...
//typedef __SIZE_T                m_size_t;
#if defined(MACH_WORD_SIZE) && (MACH_WORD_SIZE == 8)
typedef int64_t                 m_word_t;
#define MACH_WORD_SIZE_LOG2     3
#else
...
#if defined(TAIKA_USE_STDIO)
#include <stdio.h>
#define congetchar()            getchar()
...
void
kmemcpy(void *dest, const void *src, size_t len)
{
    size_t      nleft = len;
    m_word_t   *dptr = NULL;
    m_word_t   *sptr = NULL;
...
void
kbfill(void *adr, uint8_t byte, size_t len)
{
    size_t      nleft = len;
    m_word_t   *ptr = NULL;
    m_word_t   *next;
...
size_t
kstrncpy(char *dest, const char *src, size_t len)
{
    size_t  nb = 0;
...
static size_t
_ltoxn(long val, char *buf, size_t len)
{
    uint8_t u8;
    long    l;
    long    m;
...
static long
_ltodn(long val, char *buf, size_t len)
{
    uint8_t u8;
    long    l;
    long    n;
...
{
    size_t      cnt = ofs & ((1UL << (MACH_WORD_SIZE_LOG2 + 3)) - 1);
    size_t      ndx = ofs >> (MACH_WORD_SIZE_LOG2 + 3);
    size_t      val;
...
#if defined(TAIKA_HANG_ON_PANIC)
    do {
        ;
    } while (1);
#else
