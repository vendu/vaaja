...
R_handle_x_error(Display *display, XErrorEvent *event)
{
    ...
    if ((_ignbaddrawable) && (event->error_code == BadDrawable)) {
    ...
    fprintf(stderr, "XError, request == %s, error == %s, resourceid == 0x%08lx, minor_code == %d: %s\n", ((requestcodestr) ? requestcodestr : "\"unknown\""), ((errorcodestr) ? errorstr : "\"unknown\""), (unsigned long)event->resourceid, event->minor_code, errorstr);
