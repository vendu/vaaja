...
/* 1d..1e are reserved for future use */
#define V0_JMP_OP       0x10    // jmp(adr);
#define V0_JPC_OP       0x11    // jmp(pc + adr);
#define V0_SBR_OP       0x12    // lr = pc; jmp(adr);
#define V0_RET_OP       0x13    // sp += tmp; jmp(lr);
/* MT-extension */
#define V0_SME_OP       0x14    // save(uctx, map);
                                // lr = pc; fp = fp0; sp = sp0;
                                // sp += src;
#define V0_SMX_OP       0x15    // r1 = src;
                                // fp0 = fp; sp0 = sp;
                                // pop(uctx);
#define V0_THR_OP       0x16    // push(uctx); tcb = ctx; thr(dest, adr);
#define V0_THX_OP       0x17    // trap(func, code); load(tcb);
...
/* BRANCH-unit 0x02; branch instructions */
#define V0_BRANCH_UNIT  0x02
#define V0_CMP_OP       0x11    // comparison
#define V0_UCMP_OP      0x12    // comparison
...
#define V0_WFE_OP       0x10    // wait for [write] event
#define V0_SEV_OP       0x11    // signal [write] event
#define V0_IPG_OP       0x12    // invalidate page TLB-entry
#define V0_RAND_OP      0x13    // fetch-and-AND
#define V0_RLOR_OP      0x14    // fetch-and-OR
#define V0_RXOR_OP      0x15    // fetch-and-XOR
#define V0_RINC_OP      0x16    // fetch-and-increment by one
#define V0_RDEC_OP      0x17    // fetch-and-decrement by one
#define V0_RADD_OP      0x18    // fetch-and-add
#define V0_CAS_OP       0x19    // compare-and-swap
#define V0_XCH_OP       0x1a    // atomic exchange
